...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_GUILD_LEVELING_SYSTEM:
        {
            if (Player* player = object->ToPlayer())
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_GUILD_LEVELING_SYSTEM:
        {
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outError("Guildlevel condition has invalid option (%u), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError("Guildlevel condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        default:
            break;
