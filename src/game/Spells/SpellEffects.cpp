    ...
    else
    {
        ...
        if (item_owner != p_caster && p_caster->GetSession()->HasPermission(RBAC_PERM_LOG_GM_TRADE))
        {
            if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                sLog->outTradeDB("Enchant perm", p_caster->GetSession()->GetAccountId(), itemTarget->GetEntry(), 1, 0, item_owner->GetSession()->GetAccountId());
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    {
        if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
            sLog->outTradeDB("Enchant temp", p_caster->GetSession()->GetAccountId(), itemTarget->GetEntry(), 1, 0, item_owner->GetSession()->GetAccountId());
