...
void WorldObject::DelEventVisibleFor(Player* player)
{
    ...
    if (group)
    {
        for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
            if (Player* member = groupRef->GetSource())
                _EventVisibilityPlayers.erase(member->GetGUID());
    }
    else
        _EventVisibilityPlayers.erase(player->GetGUID());
}
    ...
    for (EventVisible::const_iterator itr = _EventVisibilityPlayers.begin(); itr != _EventVisibilityPlayers.end(); itr++)
    {
        ...
        if (forPlayer)
        {
            ...
            if (obj->ToPlayer()->GetGUID() == (*itr).first)
                return (*itr).second;
            // Mientras no lo encontremos sera invisible para este
            else if ((*itr).second)
                isVisible = false;
        }
        // Cuando quiere ver a una Unidad o Objeto
        else if (!obj->_EventVisibilityPlayers.empty())
        {
            for (EventVisible::const_iterator itr2 = obj->_EventVisibilityPlayers.begin(); itr2 != obj->_EventVisibilityPlayers.end(); itr2++)
            {
                // Si tiene al menos un jugador en comun, se pueden ver entre ellos, siempre que los dos puedan ver al jugador
                if ((*itr).first == (*itr2).first)
                    return (*itr).second == true && (*itr).second == (*itr2).second;
            }
        }
    }
...
bool WorldObject::IsBuildEventVisibleFor(const WorldObject* obj) const
{
    ...
    if (obj->ToCreature())
        event = sEventMgr->GetEventByCreatureGUID(obj->GetGUID(), true);

    if (obj->ToGameObject())
        event = sEventMgr->GetEventByGameObjectGUID(obj->GetGUID(), true);

    if (!event)
        return true;

    return (ToCreature() && event->IsEventCreature(GetGUID())) ||
        (ToGameObject() && event->IsEventGameObject(GetGUID())) ||
