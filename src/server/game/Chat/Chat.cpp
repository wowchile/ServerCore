            ...
            if (!AccountMgr::IsPlayerAccount(m_session->GetSecurity()))
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                {
                    ...
                    if (player->GetSelectedUnit())
                        sel_guid = player->GetSelectedUnit()->GetGUID();

                    if (sel_guid == player->GetGUID())
                        sel_guid = 0;
                    std::string command;
                    ...
                    bool HasTarget = player->GetSelectedUnit() && sel_guid != 0; 
                    sLog->outCommandDB(command.c_str(), args.c_str(), m_session->GetAccountId(), player->GetGUIDLow(), player->GetPositionX(), 
                        player->GetPositionY(), player->GetPositionZ(), player->GetMapId(), HasTarget ? (uint32)player->GetSelectedUnit()->GetTypeId() : TYPEID_PLAYER, HasTarget ? player->GetSelectedUnit()->GetName().c_str() : "", HasTarget ? player->GetSelectedUnit()->GetGUIDLow() : 0); 
                }
