...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    if (ipCountry && strcmp(sWorld->GetCountryRestrict(), "all") != 0 && strcmp(ipCountry->GetCountryCode(), sWorld->GetCountryRestrict()) != 0)
    {
        packet.Initialize (SMSG_AUTH_RESPONSE, 1);
        packet << uint8 (AUTH_REJECT);
        ...
        SendPacket(packet);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        sLog->outStaticDebug(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSessionCallback: Country restrict, denying client (%s).", GetRemoteAddress().c_str());
#endif
        CloseSocket();
        return -1;
    }
