...
class command_go_move : public CommandScript
{
public:
    command_go_move() : CommandScript("command_go_move")
    {
    }
...
std::set<uint32> command_go_move::GObjects;
UNORDERED_MAP<uint64, uint32> command_go_move::SpawnQue;

// possible spells:
// 27651, 897

class spell_place_go_move : public SpellScriptLoader
{
public:
    spell_place_go_move() : SpellScriptLoader("spell_place_go_move") { }

    class spell_place_go_moveSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_place_go_moveSpellScript);
        ...
        void HandleAfterCast()
        {
            ...
            if (summonPos && command_go_move::SpawnQue.find(player->GetGUID()) != command_go_move::SpawnQue.end())
                command_go_move::SpawnObject(player, summonPos->GetPositionX(), summonPos->GetPositionY(), summonPos->GetPositionZ(), player->GetOrientation(), player->GetPhaseMaskForSpawn(), false, command_go_move::SpawnQue[player->GetGUID()], false, true);
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_place_go_moveSpellScript::HandleAfterCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_place_go_moveSpellScript();
    }
};
